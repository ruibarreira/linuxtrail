...
WebInspector.FlameChartDelegate.prototype={requestWindowTimes:function(startTime,endTime){},updateBoxSelection:function(startTime,endTime){}}
...
{delete this._cancelWindowTimesAnimation;},_startCanvasDragging:function(event)
{if(event.shiftKey){this._startBoxSelection(event);this._isDragging=true;return true;}
...
{if(this._isSelecting){this._updateBoxSelection(event);return;}
...
{this._hideBoxSelection();this._isDragging=false;},_startBoxSelection:function(event)
{this._selectionOffsetShiftX=event.offsetX-event.pageX;this._selectionOffsetShiftY=event.offsetY-event.pageY;this._selectionStartX=event.offsetX;this._selectionStartY=event.offsetY;this._isSelecting=true;var style=this._selectionOverlay.style;style.left=this._selectionStartX+"px";style.top=this._selectionStartY+"px";style.width="1px";style.height="1px";this._selectedTimeSpanLabel.textContent="";this._selectionOverlay.classList.remove("hidden");},_hideBoxSelection:function()
{this._selectionOffsetShiftX=null;this._selectionOffsetShiftY=null;this._selectionStartX=null;this._selectionStartY=null;this._isSelecting=false;this._selectionOverlay.classList.add("hidden");},_updateBoxSelection:function(event)
{var x=event.pageX+this._selectionOffsetShiftX;var y=event.pageY+this._selectionOffsetShiftY;x=Number.constrain(x,0,this._offsetWidth);y=Number.constrain(y,0,this._offsetHeight);var style=this._selectionOverlay.style;style.left=Math.min(x,this._selectionStartX)+"px";style.top=Math.min(y,this._selectionStartY)+"px";var selectionWidth=Math.abs(x-this._selectionStartX);style.width=selectionWidth+"px";style.height=Math.abs(y-this._selectionStartY)+"px";var timeSpan=selectionWidth*this._pixelToTime;this._selectedTimeSpanLabel.textContent=Number.preciseMillisToString(timeSpan,2);var start=this._cursorTime(this._selectionStartX);var end=this._cursorTime(x);if(end>start)
this._flameChartDelegate.updateBoxSelection(start,end);else
this._flameChartDelegate.updateBoxSelection(end,start);},_onMouseMove:function(event)
...
{this._selectedEntryIndex=entryIndex;this._revealEntry(entryIndex);this._updateElementPosition(this._selectedElement,this._selectedEntryIndex);},_updateElementPosition:function(element,entryIndex)
...
return;this._enabled=enabled;this._currentPositionArea.style.cursor=enabled?"text":"";},_onMouseOver:function(entered)
{if(!this._enabled)
return;this._currentPositionElement.style.visibility=entered?"":"hidden";},_onMouseMove:function(event)
{if(!this._enabled)
return;var x=event.offsetX+event.target.offsetLeft;this._currentPositionElement.style.left=x+"px";},_resizerElementStartDragging:function(event)
{if(!this._enabled)
