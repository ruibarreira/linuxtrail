...
return false;return true;},_loadPromise:function()
{if(this._loaded)
...
return Promise.resolve();if(Runtime.isReleaseMode()){var base=this._descriptor.remote&&Runtime._remoteBase||undefined;return loadScriptsPromise([this._name+"_module.js"],base);}
...
Runtime.experiments=new Runtime.ExperimentsSupport();Runtime._remoteBase=Runtime.queryParam("remoteBase");var runtime;console=console;console.__originalAssert=console.assert;console.assert=function(value,message)
{if(value)
...
var data=this.toUint8Array();var n=data.length;var encoded="";if(n===0)
...
{var n=this.length;var dataLen=0;for(var i=0;i<n;++i){var c=this.charCodeAt(i);dataLen+=c<0x80?1:c<0x800?2:c<0x10000?3:c<0x200000?4:c<0x4000000?5:6;}
var data=new Uint8Array(dataLen);var j=0;for(var i=0;i<n;++i){var c=this.charCodeAt(i);if(c<0x80){data[j++]=c;}else if(c<0x800){data[j++]=192+(c>>>6);data[j++]=128+(c&63);}else if(c<0x10000){data[j++]=224+(c>>>12);data[j++]=128+(c>>>6&63);data[j++]=128+(c&63);}else if(c<0x200000){data[j++]=240+(c>>>18);data[j++]=128+(c>>>12&63);data[j++]=128+(c>>>6&63);data[j++]=128+(c&63);}else if(c<0x4000000){data[j++]=248+(c>>>24);data[j++]=128+(c>>>18&63);data[j++]=128+(c>>>12&63);data[j++]=128+(c>>>6&63);data[j++]=128+(c&63);}else{data[j++]=252+(c>>>30);data[j++]=128+(c>>>24&63);data[j++]=128+(c>>>18&63);data[j++]=128+(c>>>12&63);data[j++]=128+(c>>>6&63);data[j++]=128+(c&63);}}
return data;}
...
{}
WebInspector.ProgressStub.prototype={isCanceled:function()
{return false;},setTitle:function(title)
{},done:function()
{},setTotalWork:function(totalWork)
{},setWorked:function(worked,title)
{},worked:function(worked)
{},__proto__:WebInspector.Object.prototype};WebInspector.ResourceType=function(name,title,categoryTitle,color,isTextType)
...
{}
WebInspector.VersionController.currentVersion=11;WebInspector.VersionController.prototype={updateVersion:function()
...
{var setting="customDevicePresets";var newSetting="customEmulatedDeviceList";if(!window.localStorage)
return;if(!(setting in window.localStorage))
return;try{var list=JSON.parse(window.localStorage[setting]);if(!Array.isArray(list))
...
window.localStorage[newSetting]=JSON.stringify(newList);delete window.localStorage[setting];}catch(e){}},_clearBreakpointsWhenTooMany:function(breakpointsSetting,maxBreakpointsCount)
...
{WebInspector.StaticContentProvider.searchInContent(this._content,query,caseSensitive,isRegex,callback);}};WebInspector.Streams={};WebInspector.Streams._lastStreamId=0;WebInspector.Streams._boundStreams={};WebInspector.Streams.bindOutputStream=function(stream)
{WebInspector.Streams._boundStreams[++WebInspector.Streams._lastStreamId]=stream;return WebInspector.Streams._lastStreamId;}
WebInspector.Streams.discardOutputStream=function(id)
{WebInspector.Streams._boundStreams[id].close();delete WebInspector.Streams._boundStreams[id];}
...
{callback({statusCode:404});},upgradeDraggedFileSystemPermissions:function(fileSystem)
...
return WebInspector._monospaceFontFamily;};WebInspector.UserMetrics=function()
...
WebInspector.UserMetrics._ActionCodes={WindowDocked:1,WindowUndocked:2,ScriptsBreakpointSet:3,TimelineStarted:4,ProfilesCPUProfileTaken:5,ProfilesHeapProfileTaken:6,AuditsStarted:7,ConsoleEvaluated:8,FileSavedInWorkspace:9,DeviceModeEnabled:10,AnimationsPlaybackRateChanged:11,RevisionApplied:12}
...
{DevToolsAPI.sendMessageToEmbedder("loadNetworkResource",[url,headers,streamId],(callback));},setInjectedScriptForOrigin:function(origin,script)
