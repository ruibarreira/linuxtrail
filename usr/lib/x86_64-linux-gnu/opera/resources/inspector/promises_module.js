...
WebInspector.PromisePane._maxPromiseCount=10000;WebInspector.PromisePane.prototype={elementsToRestoreScrollPositionsFor:function()
...
var node=(this._promiseIdToNode.get(details.id));var wasVisible=!previousDetails||this._filter.shouldBeVisible(previousDetails,node);if(eventType==="gc"&&node&&node.parent&&!this._filter.shouldHideCollectedPromises())
...
{var promiseIdToDetails=(this._promiseDetailsByTarget.get(this._target));var currentDetails=details;while(currentDetails){var parentId=currentDetails.parentId;if(typeof parentId!=="number")
...
{var node=this._promiseIdToNode.get(details.id);if(!node){node=new WebInspector.PromiseDataGridNode(details,this._target,this._linkifier,this._dataGrid);this._promiseIdToNode.set(details.id,node);}else{node.update(details);}
...
return;var promiseId=node.promiseId();if(this._promiseDetailsByTarget.has(target)){var details=this._promiseDetailsByTarget.get(target).get(promiseId);if(details.__isGarbageCollected)
return;}
...
return undefined;var details=this._promiseDetailsByTarget.get(this._target).get(node.promiseId());if(!details)
...
{WebInspector.ViewportDataGridNode.call(this,{});this._details=details;this._target=target;this._linkifier=linkifier;this._linkifiedAnchors=[];this.dataGrid=dataGrid;}
WebInspector.PromiseDataGridNode.prototype={_disposeAnchors:function()
{for(var i=0;i<this._linkifiedAnchors.length;++i)
this._linkifier.disposeAnchor(this._target,this._linkifiedAnchors[i]);this._linkifiedAnchors=[];},update:function(details)
...
{this._element.classList.toggle("promise-gc",!!this._details.__isGarbageCollected);WebInspector.ViewportDataGridNode.prototype.createCells.call(this);},_appendCallFrameAnchor:function(cell,callFrame)
{if(!callFrame)
return;var anchor=this._linkifier.linkifyConsoleCallFrame(this._target,callFrame);this._linkifiedAnchors.push(anchor);cell.appendChild(anchor);},createCell:function(columnIdentifier)
...
{if(!callFrame)
return"";var script=callFrame.scriptId?this._target.debuggerModel.scriptForId(callFrame.scriptId):null;var sourceURL=script?script.sourceURL:callFrame.url;var lineNumber=callFrame.lineNumber||0;return WebInspector.displayNameForURL(sourceURL)+":"+lineNumber;},dataTextForSearch:function()
...
return false;if(this.shouldHideCollectedPromises()&&details.__isGarbageCollected)
